...
namespace Cdm.Authentication.OAuth2
{
    ...
    public abstract class AuthorizationCodeFlow : IDisposable
    {
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            // print log
            Debug.Log($"qqqqqq 1111111 ExchangeCodeForAccessTokenAsync {authorizationResponseUri}");
            // Is there any error?
            ...
            // print log
            Debug.Log($"qqqqqq 222222 ExchangeCodeForAccessTokenAsync 22222authorizationResponse");
        ...
        public virtual async Task<string> ExtractAuthorizationCode(string redirectUrl, CancellationToken cancellationToken = default) {
            ...
            // print log
            Debug.Log($"qqqqqq 1111111 ExtractAuthorizationCode {authorizationResponseUri}");
            // Is there any error?
            if (JsonHelper.TryGetFromNameValueCollection<AuthorizationCodeRequestError>(query, out var authorizationError))
                throw new AuthorizationCodeRequestException(authorizationError);
            ...
            // print log
            Debug.Log($"qqqqqq 222222 ExtractAuthorizationCode 22222authorizationResponse");
            // Validate authorization response state.
            if (!string.IsNullOrEmpty(state) && state != authorizationResponse.state)
                throw new SecurityException($"Invalid state got: {authorizationResponse.state}");

            // print log
            Debug.Log($"qqqqqq 333333 ExtractAuthorizationCode 33333authorizationResponse {authorizationResponse.code}");
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            // print key and value of content
            foreach (var keyValuePair in content.Headers)
            {
                Debug.Log($"qqqqqqqq 00000GetAccessTokenInternalAsync {keyValuePair.Key} {keyValuePair.Value}");
            }

            // print token request
            Debug.Log($"qqqqqqqq 111111111GetAccessTokenInternalAsync {tokenRequest}");
            Debug.Log($"qqqqqqqq 2222222222222222GetAccessTokenInternalAsync {await tokenRequest.Content.ReadAsStringAsync()}");
            ...
            // print log
            Debug.Log($"qqqqqqqq 555555555GetAccessTokenInternalAsync {error}");
