    ...
    public class AuthenticationSession : IDisposable
    {
        ...
        public async Task<AccessTokenResponse> AuthenticateAsync(CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                Debug.Log("Getting authorization grant using browser login..." + authorizationUrl);
        ...
        // create a new authenticate method that returns a authorization code
        public async Task<string> AuthenticateAsyncForAuthorizationCode(CancellationToken cancellationToken = default)
        {
            using var timeoutCancellationTokenSource = new CancellationTokenSource(loginTimeout);
            ...
            try
            {
                ...
                if (browserResult.status == BrowserStatus.Success)
                {
                    ...
                    Debug.Log($"Redirect URL: {browserResult.redirectUrl}");
#endif
                    return await _client.ExtractAuthorizationCode(browserResult.redirectUrl, cancellationToken);
                }
